...
export default class XlsxFileView extends ConvertibleFileView {
  ...
  async getMarkdownContent(
    ...
    try {
      ...
      console.error("Failed to read XLSX file", this.file.path, e)
      return `(Error reading file: ${this.file.basename})`
    }

    // FileSizeGate: >50MB
    if (buffer.byteLength > SIZE_GATE_BYTES) {
      return `(File too large for Markdown conversion: ${formatSize(buffer.byteLength)})`
    }

    // Dynamic import: SheetJS loaded on demand
    const XLSX = await import("xlsx")
    ...
    try {
      ...
      console.error("Failed to parse XLSX file", this.file.path, e)
      return `(Error parsing file: ${this.file.basename})`
    }

    const sheetNames: string[] = workbook.SheetNames
    if (sheetNames.length === 0) return "(no sheets)"
    ...
    for (const sheetName of sheetNames) {
      ...
      const normalized = rows.map(row => {
        ...
        return padded.map(cell => escapePipe(String(cell)))
      })
      ...
      for (let i = 1; i < normalized.length; i++) {
        sections.push("| " + normalized[i].join(" | ") + " |")
      }
