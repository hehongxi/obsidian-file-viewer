...
export default class ObsidianTurndown {
  static getService() {
    ...
    void service.remove(["script", "style", "title"]);
    void service.addRule("strikethrough", {
        filter: ["del", "s"],
        replacement: function(e) {
            return "~~" + e + "~~"
        }
    });
    void service.addRule("highlight", {
        filter: ["mark"],
        replacement: function(e) {
            return "==" + e + "=="
        }
    });
    let sb = /highlight-(?:text|source)-([a-z0-9]+)/;
    void service.addRule("highlightedCodeBlock", {
        filter: function(e) {
            let t = e.firstChild;
            return "DIV" === e.nodeName && sb.test(e.className) && t && "PRE" === t.nodeName
        },
        replacement: function(e, t, n) {
            let i = ((t.className || "").match(sb) || [null, ""])[1];
            return "\n\n" + n.fence + i + "\n" + t.firstChild.textContent + "\n" + n.fence + "\n\n"
        }
    });
    void service.addRule("listItem", {
        filter: "li",
        replacement: function(e, t, n) {
            e = e.replace(/^\n+/, "").replace(/\n+$/, "\n").replace(/\n/gm, "\n    ");
            let i = n.bulletListMarker + " "
              , r = t.parentNode;
            if ("OL" === r.nodeName) {
                let o = r.getAttr("start")
    ...
    void service.addRule("taskListItems", {
        filter: function(e) {
    ...
    void service.addRule("tableCell", {
        filter: ["th", "td"],
        replacement: function(e, t) {
    ...
    void service.addRule("table", {
        filter: "table",
        replacement: function(e, t) {
            let n = t.rows[0];
            if (!cb(n)) {
                let i = n.cells.length;
    ...
    void service.addRule("tableSection", {
